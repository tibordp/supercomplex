	...
	for (size_t i = 0; i < states.size(); ++i) {
		...
		for (auto&& transition : state.transitions) {
			...
			if (i != transition.next) {
				out << "                        self.state = " << transition.next << ";" << std::endl;
			}
			out << "                    }" << std::endl;
		}
		...
		if (state.terminal) {
			...
			out << indent << "                    self.state = " << automaton.start() << ";" << std::endl;
			...
			if (i == automaton.start()) {
				...
				out << "                        bail!(" << error_type_class << "::Eof);" << std::endl;
			}
			out << "                    } else {" << std::endl;
			out << "                        bail!(" << error_type_class << "::Unexpected);" << std::endl;
			out << "                    }" << std::endl;
		}
		out << "                }," << std::endl;
	}
