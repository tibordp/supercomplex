...
// This is the sruct that holds the information about our accepting states in
// DFA
...
void ranges(std::basic_ostream<char>& out,
    ...
    boost::icl::interval_set<char> range)
{
	...
	for (auto&& interval : range)
	{
		...
		if (interval.lower() == interval.upper())
			...
			out << "(" << name << " >= " << (long long)interval.lower() << " && "
			    << name << " <= " << (long long)interval.upper() << ")";
		first = false;
	}
	if (boost::icl::interval_count(range) > 1)
		out << ")";
...
int codegen(std::basic_ostream<char>& out,
    const supercomplex::dfa<char, c_token>& automaton)
{
	// We use a bimap so that we can give nice descriptive numeric labels to
	// states.
	...
	for (auto&& state : state_names.right)
	{
		...
		for (auto&& transition : state.second->transitions)
		{
			...
			out << std::endl
			    << "          state = " << state_names.left.at(transition.next) << ";"
			    << std::endl;
			first = false;
		}
		if (!first)
		...
		if (state.second->terminal())
		{
			...
			out << "          ungetc(ch, stdin);"
			    << std::endl; // We have to re-evaluate current character.
			out << "          buf[buf_pos] = '\\0';" << std::endl;
			if (!terminal_node.skip)
				out << "          printf(\"<%s, \\\"%s\\\">\", \"" << terminal_node.name
				    << "\", buf);" << std::endl;
	...
	out << "    if (buf_pos >= 1024) { fputs(\"Token too long.\", stderr);  "
	       "return -1; }"
	    << std::endl;
	out << "    if (feof(stdin)) { return 0; }" << std::endl;
	out << "  }" << std::endl;

	out << "  fail:" << std::endl; // If we reached an EOF, we don't care that the
	                               // EOF character could not match
	out << "  if (feof(stdin)) { return 0; }" << std::endl;
