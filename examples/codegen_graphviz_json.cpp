...
int graphviz_codegen(std::basic_ostream<char>& out, const supercomplex::lexer<char, t_info>& automaton)
{
	...
	for (size_t i = 0; i < states.size(); ++i)
	{
		...
		else if (state.terminal) 
		{
			...
			{
				out << "    " << i << " [label=\"SKIP\", peripheries=2, style=dotted];" << std::endl;
			}
			else
			{
				out << "    " << i << " [label=\"" << state.terminal_info.name << "\", peripheries=2];" << std::endl;
			}
		}	
	}
