...
namespace supercomplex {
	...
	struct terminal_node {
		...
		terminal_node(const TokenInfo& token, const std::string& regex) : token(token), regex(regex) {
		}
	};
	...
	std::shared_ptr<regex_node<CharType, TokenInfo>> parse_term(T& begin, T end) {
		...
		switch (*begin++) {
			case '+':
				return std::make_shared<operand<CharType, TokenInfo>>(char_range, operator_type::plus);
			case '*':
				return std::make_shared<operand<CharType, TokenInfo>>(char_range, operator_type::star);
			case '?':
				return std::make_shared<operand<CharType, TokenInfo>>(char_range, operator_type::optional);
			default:
				--begin;
				return char_range;
		}
