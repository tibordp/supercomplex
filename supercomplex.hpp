...
template <typename AdditionalInfo> struct basic_token_info {
  int precedence;
...
template <typename CharType, typename AdditionalInfo> struct lexer_transition {
...
template <typename CharType, typename AdditionalInfo> struct lexer_node {
...
template <typename CharType, typename AdditionalInfo> class lexer {
...
template <typename CharType, typename AdditionalInfo> struct lexer_production {
...
template <typename CharType, typename AdditionalInfo> class lexer_generator {
...
public:
  ...
  friend lexer_generator &
  operator<<(lexer_generator &lhs,
  ...
  lexer<CharType, AdditionalInfo> generate() {
    nfa<CharType, token_info_type> nfa_machine(productions.begin(),
                                               productions.end());
    ...
    for (const auto &state : dfa_machine.states()) {
      for (const auto &transition : state->transitions) {
        nodes[index[state]].transitions.emplace_back(transition.characters,
                                                     index[transition.next]);
      }
    }

    return lexer<CharType, AdditionalInfo>(nodes.begin(), nodes.end(), index[dfa_machine.start()]);
  }
};
}
